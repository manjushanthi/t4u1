...
namespace SolvencyII.Data.CRT.ETL.MappingControllers.DataPoints
{
    class DataPointsSerializer
    {
        ...
        string[] ddls = new string[]{@"CREATE TABLE mDataPoint (DataPointId INTEGER PRIMARY KEY, DimensionId INTEGER REFERENCES mDimension (DimensionID), MemberId INTEGER REFERENCES mMember (MemberID), ParentDataPointId INTEGER);"
...
left outer join (select * from mCellPosition cpx 
                ...
                inner join mAxis ax on ax.AxisID = aox.AxisID and ax.AxisOrientation = 'X') qx 
...
left outer join (select * from mCellPosition cpy 
                ...
                inner join mAxis ay on ay.AxisID = aoy.AxisID and ay.AxisOrientation = 'Y') qy 
