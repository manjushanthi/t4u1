...
namespace SolvencyII.Data.Shared
{
    public class ArelleCsSaver : ArelleCsShared
    {
        ...
        static Regex nilTypedDimPattern = new Regex(@"[<](\w+)[:](\w+)[/][>]", RegexOptions.Compiled);

        BackgroundWorker asyncWorker = null;
        Dictionary<string, string> scheamRefSubstitutions = null;
            ...
            if (!string.IsNullOrEmpty(dims))
            {
                ...
                foreach (string dimVal in dims.Split(new char[] { '|' }))
                {
                    ...
                    else // is typed
                    {
                        ...
                        writer.WriteAttributeString("dimension", dimQn);
                        if (nilTypedDimPattern.IsMatch(mem))
                        {
        ...
        public object saveXbrl(string xbrlFilePath, BackgroundWorker asyncWorker)
        {
            ...
            try
            {
                ...
                { // check if default prefix can be used for xbrli
                    if (string.IsNullOrEmpty(ns.Prefix) && ns.Namespace == nsXbrli)
                    {
                        nsXbrliPrefix = null;
                        break;
                    }
                }
                if (string.IsNullOrEmpty(nsXbrliPrefix))
                    writer.WriteStartElement("xbrl", nsXbrli);
                else
                    writer.WriteStartElement(nsXbrliPrefix, "xbrl", nsXbrli);
                foreach (NamespaceDefinition ns in dpmPrefixedNamespaces)
                {
                ...
                IEnumerable<FilingIndicator> filingIndicators = _conn.Query<FilingIndicator>(
                    "SELECT mToT.TemplateOrTableCode, dFI.Filed " +
                ...
                if (filingIndicators.Count() > 0)
                {
                    ...
                    string cntxKey = string.Format("{0}#{1}#{2}", "", instance.EntityIdentifier, instance.PeriodEndDateOrInstant);
                    cntxTbl[cntxKey] = "c";
                    cntxMd5tbl[cntxKey] = fiCntxMd5sum;
                    ...
                    foreach (FilingIndicator r in filingIndicators)
                    {
                        ...
                        if (r.Filed.HasValue && !r.Filed.Value)
                            writer.WriteAttributeString("filed", nsFind, "false");
                ...
                foreach (Fact f in factsInInstance)
                {
                    ...
                    List<object> _toHash = new List<object>();
                    if (dpSigParts.Groups.Count != 5 || dpSigParts.Groups[2].Value.Length == 0)
                    {
                    ...
                    if (!xmlnsPrefixNS.ContainsKey(conceptPrefix))  
                    {
                        logError("sqlDB:InvalidFactConceptPrefix",
                                 string.Format("Fact DPM metric prefix namespace undefined, fact ignored: {0}",
                                               dpSig),
                                 dpSig);
                        continue;
                    }
                    ...
                    if (isNumeric && !string.IsNullOrEmpty(f.Unit))
                    {
                        ...
                        else
                        {
                            ...
                            {
                                if (xmlnsPrefixNS.ContainsKey(""))
                                {
                                    prefix = "";  // allowable default if declared
                                }
                                else
                                {
                                    prefix = "xbrli"; // default prefix becomes explicitly xbrli
                                    _unitMeasure = "xbrli:" + _unitMeasure;  // must add prefix
                                }
                            ...
                            else
                            {
                                ...
                                writeUnit(writer, unitId, _unitMeasure);
